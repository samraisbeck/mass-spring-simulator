...
class MassSpring(object):
    def __init__(self, springs, damping, mass, initPos, speedPercent, forcingFunction, direction, lengthOfSim):
        ...
        self.block = pygame.transform.scale(
            self.block, (self.blockW, self.blockH))
        # Block's resting point (middle):
    ...
    def runNumericalMethod(self, iterations=100000):
        ...
        a little messy."""
        sampleRate = self.speedPercent * (iterations / (1000)) * 10 / \
            self.length  # only sample every 1000 points.
        ...
        for i in range(1, iterations + 1):
            ...
            k1v = (forcingFunction - self.b *
                   v[i - 1] - self.k * y_t[i - 1]) / self.m  # k1v is dv/dt
    ...
    def analytical(self, iterations=100000):
        ...
        for i in range(1, iterations):
            ...
            yAct[i] = np.exp(-(1 / 3.0) * tAct[i]) * (2 * np.cos((sqrt(29) / 3) *
                                                                 tAct[i]) + (2 / sqrt(29)) * np.sin((sqrt(29) / 3) * tAct[i]))  # num3
            # yAct[i] = 2*np.cos((sqrt(1300)/sqrt(3))*tAct[i]) # num4
            # yAct[i] = 44/7.0*np.cos(sqrt(2)*tAct[i])-30/7.0*np.cos(3*tAct[i]) #num5
            if i % sampleRate == 0:
                data.write(
                    str(round(abs(yAct[i] - self.y[i / sampleRate]), 6)) + '\n')
        data.close()
    ...
    def update(self, frame):
        ...
        if str(self.t[frame]).split('.')[0] != self.printTime:
            ...
            self.timeText = self.renderText(
                'Time: ' + self.printTime, 40, (255, 0, 0))
        ...
        if (self.direction == "X"):
            for i in range(1, self.maxInitPos + 1):
                ...
                pygame.draw.line(self.window, (0, 255, 0), ((
                    WIDTH / 2) + (100 * i), self.blockY - 20), ((WIDTH / 2) + (100 * i), self.blockY + self.blockH + 20))
                self.window.blit(
                    self.distanceTexts[2 * (i - 1)], ((WIDTH / 2) + (100 * i), self.blockY + self.blockH + 20))
                pygame.draw.line(self.window, (0, 255, 0), ((
                    WIDTH / 2) - (100 * i), self.blockY - 20), ((WIDTH / 2) - (100 * i), self.blockY + self.blockH + 20))
                self.window.blit(
                    self.distanceTexts[2 * i - 1], ((WIDTH / 2) - (100 * i), self.blockY + self.blockH + 20))
            # print round(self.y[frame]*100)
            pygame.draw.line(self.window, (255, 0, 0), (WIDTH / 2, self.blockY - 20),
                             (WIDTH / 2, self.blockY + self.blockH + 20))  # Equilibrium line
            self.window.blit(
                self.distanceTexts[-1], (WIDTH / 2, self.blockY + self.blockH + 20))
            ...
            for i in range(len(self.springs)):
                ...
                if type(self.springs[i]) == list:
                    ...
                    for j in range(len(self.springs[i])):
                        ...
                        pygame.draw.line(self.window, (abs(cos(j * (pi / 2))) * 255, 0, 150), (startPos, ((-1)**i * (
                            i * 10)) + HEIGHT / 2), (startPos + perc * dist, ((-1)**i * (i * 10)) + HEIGHT / 2), 5)
                        startPos += perc * dist
                else:
                    pygame.draw.line(self.window, (255, 0, 150), (WIDTH / 2, ((-1)**i * (i * 10)) +
                                                                  HEIGHT / 2), (self.blockX + self.blockW / 2, ((-1)**i * (i * 10)) + HEIGHT / 2), 5)
        ...
        # copied it and made the changes in the second function. Feel free to make any modifications to improve this.
            ...
            for i in range(1, self.maxInitPos + 1):
                # Draw the measurement lines (lines are spaced out 1 meter)
                pygame.draw.line(self.window, (0, 255, 0), (self.blockX - 20, HEIGHT / 2 + 100 *
                                                            i - const), (self.blockX + self.blockW + 20, HEIGHT / 2 + 100 * i - const))
                self.window.blit(
                    self.distanceTexts[2 * i - 1], (self.blockX - 20, HEIGHT / 2 + 100 * i - const))
                pygame.draw.line(self.window, (0, 255, 0), (self.blockX - 20, HEIGHT / 2 - 100 *
                                                            i - const), (self.blockX + self.blockW + 20, HEIGHT / 2 - 100 * i - const))
                self.window.blit(
                    self.distanceTexts[2 * (i - 1)], (self.blockX - 20, HEIGHT / 2 - 100 * i - const))
            ...
            # print round(self.y[frame]*100)
            pygame.draw.line(self.window, (255, 0, 0), (self.blockX - 20, HEIGHT / 2 - const),
                             (self.blockX + self.blockW + 20, HEIGHT / 2 - const))  # Equilibrium line
            self.window.blit(
                self.distanceTexts[-1], (self.blockX - 20, HEIGHT / 2 - const))
            ...
            for i in range(len(self.springs)):
                ...
                if type(self.springs[i]) == list:
                    ...
                    for j in range(len(self.springs[i])):
                        perc = (1 / self.springs[i][j]) / Sum
                        pygame.draw.line(self.window, (abs(cos(j * (pi / 2))) * 255, 0, 150), (((-1)**i * (i * 10)) + WIDTH / 2,
                                                                                               startPos - const), (((-1)**i * (i * 10)) + WIDTH / 2, startPos + perc * dist - const), 5)
                        startPos += perc * dist
                else:
                    pygame.draw.line(self.window, (255, 0, 150), ((-1)**i * (i * 10) + WIDTH / 2, HEIGHT /
                                                                  2 - const), (((-1)**i * (i * 10)) + WIDTH / 2, self.blockY + self.blockH / 2 - const), 5)
...
if __name__ == '__main__':
    ...
    MassSpringSim = MassSpring(totalSprings, damping, mass, pos0, percSpeed, fNum, direction,
                               lengthOfSim)
    # Run the calculation
    MassSpringSim.runNumericalMethod(
        int(MassSpringSim.length / MassSpringSim.inc))
    ...
    while True:
        ...
        if run == False:
            ...
            replayText = font.render(
                'Click Space to Replay or esc to Exit', 1, (0, 0, 125))
